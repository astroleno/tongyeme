...
function createScopedCraneScene(stage) {
...
export function initCraneRouteEntry(stage = document.querySelector('[data-crane-stage]')) {
  if (!stage) return null;
  if (stage.__craneRouteEntryScene) return stage.__craneRouteEntryScene;

  const sceneState = createScopedCraneScene(stage);
  if (!sceneState) return null;

  let fallbackDestroy = null;
  const route = createTransitionRoute({
    name: 'Crane transition component',
    root,
    body: document.body,
    stage,
    smoothOptions: {
    ...
    onError: (error, context) => {
      console.warn('Crane transition component fell back to native scroll sync.', error);
  ...
  const scene = {
    ...
    destroy() {
      ...
      route.destroy();
      sceneState.destroy();
      mountedScenes.delete(scene);
      if (stage.__craneRouteEntryScene === scene) {
        delete stage.__craneRouteEntryScene;
      }
    },
    renderRawProgress: sceneState.renderRawProgress
  };

  stage.__craneRouteEntryScene = scene;
  mountedScenes.add(scene);
  return scene;
}

document.querySelectorAll('[data-crane-stage]').forEach((stage) => initCraneRouteEntry(stage));

window.addEventListener('pagehide', () => {
  [...mountedScenes].forEach((scene) => scene.destroy());
});
